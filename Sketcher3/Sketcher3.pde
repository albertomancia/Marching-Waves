...
String filename = "blondemask2.png";
  ...
  if(render){
    ...
    if(narrowBand.isEmpty()){
      ...
      if(!showField){
        ...
        loadPixels();
        for(int i = 0; i < pixels.length; i ++){
          if(fieldNo == 0) pixels[i] = color(T[i]);
...
void initialize(){
  ...
  for(int i = 0; i < width * height; i ++){
    ...
    F[i] = map(brightness(sample), brightness(line), brightness(bg), 0.15, 2);
